...
ripple-tools set-flag defaultRipple \
    ...
    --secret $COLD_SECRET \
    --sequence 1 --format json --offline --export >> transactions.json
ripple-tools set-flag requireDestinationTag \
    --address $COLD_ADDRESS \
    --secret $COLD_SECRET \
    --sequence 2 --format json --offline --export >> transactions.json
ripple-tools set-flag disallowIncomingXRP \
    --address $COLD_ADDRESS \
    --secret $COLD_SECRET \
    --sequence 3 --format json --offline --export >> transactions.json
